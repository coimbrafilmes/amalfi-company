...
- [x] **I6 (MEDIUM, 1h):** `drawSeal` octogonal premium dourado + aplicar em slots premium. Ref: 04_Imagens/3.png + modulo-1.png — `drawSeal(opts)` em primitives.ts: octógono regular 8 vértices via polygon SVG, dupla borda (outer + inner 88% scale opacity 0.6), texto sans600 UPPERCASE em 2-3 linhas centralizadas. Aplicado: anuncio-3 (radius 130, canto superior direito), aplus-1-header (radius 60), aplus-7-premium (radius 60). Texto fixo "Curado / Amalfi"
- [x] **I7 (LOW-MEDIUM, 1h):** aplus-cta com botão dourado destacado + 2 ícones rodapé (Garantia/Envio). Ref: modulo-6.png — slot reescrito: headline sans-bold osso 48pt + sub italic ocre 26pt (acento emocional) + drawPill dourado grande (ocre bg, ícone sparkle, fontSize 22, padding 32/18) com label "Eleve seu Ambiente" — visual destaque sem fingir CTA compra Amazon. 2 mini-ícones rodapé: shield "Garantia Amalfi" + gem "Curadoria Premium"
