...
function imageInfoFor(imagens: ImagemGerada[] | undefined, n: number, variante: 'anuncio' | 'aplus') {
              ...
              {results.briefingsAPlus && results.briefingsAPlus.length > 0 ? (
                <>
                    ...
                    {results.briefingsAPlus.map((b, idx) => {
                      ...
                      return (
                        <BriefingTile
                          ...
                          numero={b.numero}
                          titulo={b.titulo}
                          tag={String(b.estagio).replace('aplus-', '')}
                          paleta={paletaPraTile(idx, b.paletaCor)}
                          imageBase64={info.base64}
                          failed={info.failed}
                          aspect="landscape"
                  ...
                  {results.imagens && (() => {
                    ...
                    return (
                      <p className="mt-8 font-editorial italic text-base opacity-75">
                        {ok} de {list.length} imagens A+ · 970×600
                      </p>
                    );
                  })()}
                </>
                ...
                <p className="font-editorial italic text-lg opacity-60">
                  Anúncio antigo sem A+ Content. Gere um novo pra ter as 6 imagens A+.
                </p>
              )}
            </Tabs.Content>
          </Tabs.Root>
